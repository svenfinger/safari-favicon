 ...
 * @property {import('./load-png.js').ScalingMode} scaling
 */
 ...
 * @returns {CheckResult}
 */
export function buildChecklist(fileName, analysis, lightMode, darkMode, sizeInfo) {
  /** @type {Check[]} */
  const checks = [];

  checks.push({ level: 'pass', message: `PNG loaded: ${fileName}` });
  addSizeCheck(checks, sizeInfo);
 ...
 * @param {SizeInfo} [sizeInfo]
 */
function addSizeCheck(checks, sizeInfo) {
  if (!sizeInfo) {
    checks.push({
      level: 'pass',
  ...
  const { sourceWidth, sourceHeight, scaling } = sizeInfo;
  const source = `${sourceWidth}×${sourceHeight}`;

  if (scaling === 'none') {
    checks.push({
      level: 'pass',
      message: `Source is ${source} — analyzed at native Safari tab size, no resampling. Note: Retina tabs render ~2× (≈32px), so a 32×32 source stays sharper.`,
    });
    return;
  }
  ...
  if (scaling === 'upscaled') {
    checks.push({
      level: 'warn',
      message: `Source is ${source}, smaller than ${FAVICON_SIZE}×${FAVICON_SIZE}. Safari still shows it but upscales it, so it will look blurry — ship at least 32×32.`,
    });
    return;
  }
  ...
  checks.push({
    level: 'pass',
    message: `Source is ${source} — Safari accepts it and downscales to tab size. Simulated an area-average downscale to ${FAVICON_SIZE}×${FAVICON_SIZE} for analysis.`,
  });
}
